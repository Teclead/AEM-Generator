...
export class TouchUIXMLGenerator<T = {}> extends UiGenerator<T> {
  ...
  public writeFilesToAEM() {
    ...
    if (this.dialogConfig.tabs) {
      ...
      const hasCustomFunctions = this.hasHideImplementation() || this.hasOnLoadImplementation();
      ...
      if (this.existFolder(this.dialogConfig.componentPath + '/clientlibs') && !hasCustomFunctions) {
